...
Repositório com estudos de caso e projetos práticos em Machine Learning, Databricks/Unity Catalog, Engenharia de Dados com PySpark e RAG (Retrieval-Augmented Generation) com LLMs. Reúne desde experimentos de IA generativa aplicada a documentos até modelos preditivos clássicos de detecção de fraude, AutoML e comparações de performance PySpark vs SQL.
...
## ⚙️ Engenharia de Dados (PySpark & Spark SQL)

### `Pyspark vs SQL via Spark - Manipulação de Dados.ipynb`

Estudo comparativo de performance e sintaxe entre a API do PySpark e o Spark SQL, usando os dados de transações de cartão de crédito do Unity Catalog como base. Para cada operação, o notebook implementa as duas abordagens lado a lado e mede o tempo de execução:

- **JOIN simples** entre a tabela de transações e a tabela de descrição de classes.
- **JOIN + CASE WHEN**, classificando o valor (`Amount`) das transações em faixas (ex: "Aporte Muito Baixo", "Aporte Baixo").
- **Agregações com GROUP BY**, calculando estatísticas (contagem, soma, média, mínimo, máximo) por classe de transação.
- **Window Functions**, com ranking (`ROW_NUMBER`) e soma acumulada particionados por classe.

Ao final, gera um resumo comparativo do tempo de execução de cada operação nas duas abordagens, servindo como referência prática de quando usar a API do PySpark ou Spark SQL direto.
...
- **Dados e infraestrutura**: Databricks · PySpark · Spark SQL · Unity Catalog · pandas · Matplotlib · Seaborn
...
Os notebooks de Machine Learning e Engenharia de Dados foram desenvolvidos e executados em ambiente Databricks, utilizando tabelas do Unity Catalog como fonte de dados.
